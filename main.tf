...
resource "aws_security_group" "destination" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      to_port     = 443
      protocol    = "tcp"
      cidr_blocks = local.firehose_cidr_blocks[local.destination][data.aws_region.current.region]
      description = "Allow inbound traffic from Kinesis Firehose"
    }
  }
...
resource "aws_security_group_rule" "destination" {
  ...
  to_port                  = 443
  protocol                 = "tcp"
  cidr_blocks              = !local.is_search_destination ? local.firehose_cidr_blocks[local.destination][data.aws_region.current.region] : null
