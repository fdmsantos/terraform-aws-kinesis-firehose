locals {
  ...
  add_sse_kms_policy                = var.create && var.create_role && !local.is_kinesis_source && var.enable_sse && var.sse_kms_key_type == "CUSTOMER_MANAGED_CMK"
  ...
  msk_source_topic_arn              = local.add_msk_source_policy ? "arn:aws:kafka:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:topic/${data.aws_msk_cluster.this[0].cluster_name}/${data.aws_msk_cluster.this[0].cluster_uuid}/${var.msk_source_topic_name}" : null
  msk_source_group_arn              = local.add_msk_source_policy ? "arn:aws:kafka:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:group/${data.aws_msk_cluster.this[0].cluster_name}/${data.aws_msk_cluster.this[0].cluster_uuid}/*" : null
}
...
data "aws_iam_policy_document" "kinesis" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      condition {
        test     = "StringEquals"
        values   = ["kinesis.${data.aws_region.current.region}.amazonaws.com"]
